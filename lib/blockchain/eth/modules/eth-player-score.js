...
//            gas = '0x' + gas.toString(16)
    ...
    async getTopScoresSecure (index, count) {
        try {
            ...
            let topScores = []
            for (; count; count--, index++) {
                let player = await m.getTopPlayerAddress(index).call()
                let score = await m.getTopPlayerScore(index).call()
                topScores.push({
                    player,
                    score
                })
...
//            gas += 1
            var msg = `${address}${score}${metrics}`
            let result1 = await m.verifySign(msg, v, r, s).call({
                from, 
                gas: 4712388
            })
