class EthAssetValidator {
    ...
    async validateMetrics (puzzleId, metrics) {
        try {
            let from = this.options.contracts.PuzzleManager.options.from
            let gas = await this.puzzleManager.methods.ValidateMetrics(puzzleId, metrics).estimateGas()
            let result = await this.puzzleManager.methods.ValidateMetrics(puzzleId, metrics).send({
                from, 
                gas
            })
            return {
